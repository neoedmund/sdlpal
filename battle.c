...
WORD
g_rgPlayerPos [ 3 ] [ 3 ] [ 2 ] = { { { 240 , 170 } } , // one player
...
--*/ {
	...
	// neoe: auto save, workflow not work for  after battle, so save before battle for easy
	PAL_SaveGame ( va ( "%s%d%s" , PAL_SAVE_PREFIX , 5 , ".rpg" ) , 1 ) ;

	stealCnt = 0 ;
	if ( fIsBoss ) g_Battle . UI . fAutoAttack = FALSE ;
